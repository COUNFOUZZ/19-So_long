...
void	ft_init_img(t_game *g)
{
	...
	g->img.player_exit = mlx_xpm_file_to_image(g->mlx, "./img/player_exit.xpm", &g->img.img_width, &g->img.img_height);
	...
	g->img.dopen = mlx_xpm_file_to_image(g->mlx, "./img/dopen.xpm", &g->img.img_width, &g->img.img_height);
	...
	while (g->map.map[++i])
	{
		...
		while (g->map.map[i][++j])
		{
			if (g->map.map[i][j] == '1')
				...
				ft_put_img(g, i, j, '0');
			else if (g->map.map[i][j] == 'P' && g->player.pos_y == g->check.exit_pos_y && g->player.pos_x == g->check.exit_pos_x && g->check.nbr_coin != g->player.coin_collected)
				ft_put_img(g, i, j, 'p');
			else if (g->map.map[i][j] == 'P')
				ft_put_img(g, i, j, 'P');
			else if (g->map.map[i][j] == 'C')
				ft_put_img(g, i, j, 'C');
			else if (g->map.map[i][j] == 'E' && g->check.nbr_coin == g->player.coin_collected)
				ft_put_img(g, i, j, 'e');
